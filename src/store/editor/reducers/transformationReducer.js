...
const initialState = {
  ...
  rotation: 0,
  resizing: false,
  isButtonClicked:{cropButton:false, resizeButton:false, rotateButton:false}
};
...
export const transformationReducer = createReducer(initialState, {
  ...
  transform:(state, action)=>{
    if(action.payload.buttonType === 'crop'){
      state.isButtonClicked={...state.isButtonClicked,cropButton:action.payload.value}
    }
    else if(action.payload.buttonType === 'resize'){
      state.isButtonClicked={...state.isButtonClicked,resizeButton:action.payload.value}
    }
    else{
      state.isButtonClicked={...state.isButtonClicked,rotateButton:action.payload.value}
    }
